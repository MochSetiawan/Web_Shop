...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (menuToggle) {
        menuToggle.addEventListener('click', function() {
            vendorSidebar.classList.toggle('show');
            
            // Add overlay when sidebar is shown on mobile
            if (window.innerWidth < 992) {
                if (vendorSidebar.classList.contains('show')) {
                    const overlay = document.createElement('div');
                    overlay.className = 'sidebar-overlay';
                    document.body.appendChild(overlay);
                    
                    overlay.addEventListener('click', function() {
                        vendorSidebar.classList.remove('show');
                        this.remove();
                    });
                } else {
    ...
    if (productForm) {
        ...
        let variationCount = document.querySelectorAll('.variation-row').length;
        ...
        if (addVariationBtn && variationsContainer) {
            addVariationBtn.addEventListener('click', function() {
                ...
                row.className = 'row variation-row mb-3';
                row.innerHTML = `
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="variation[${variationCount}][name]" placeholder="e.g. Color, Size" required>
                    </div>
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="variation[${variationCount}][value]" placeholder="e.g. Red, XL" required>
                    </div>
                    <div class="col-md-3">
                        <input type="number" class="form-control" name="variation[${variationCount}][price]" placeholder="Additional price" min="0" step="0.01">
                    </div>
                    <div class="col-md-1">
        ...
        // Sale price toggle
        const saleCheckbox = document.getElementById('has-sale-price');
        const salePriceGroup = document.getElementById('sale-price-group');
        
        if (saleCheckbox && salePriceGroup) {
            saleCheckbox.addEventListener('change', function() {
                if (this.checked) {
                    salePriceGroup.classList.remove('d-none');
                } else {
                    salePriceGroup.classList.add('d-none');
                }
            });
